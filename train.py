...
parser.add_argument("--num_classes",   type=int, help="분류할 클래수 개수 설정", default=7)
...
class Train():
    ...
    def configuration_dataset(self):
        self.train_dataset_config = DatasetGenerator(self.DATASET_NAME, self.DATASET_DIR, self.IMAGE_SIZE, self.BATCH_SIZE, mode='train')
        self.valid_dataset_config = DatasetGenerator(self.DATASET_NAME, self.DATASET_DIR, self.IMAGE_SIZE, self.BATCH_SIZE, mode='validation')
    ...
    def __set_args(self):
        self.MODEL_PREFIX = self.args.dataset_name + '_' + self.args.model_prefix
    ...
    def configuration_model(self):
        self.model = age_estimation_model(image_size=self.IMAGE_SIZE, num_classes=self.NUM_CLASSES)
